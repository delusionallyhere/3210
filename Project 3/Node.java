...
//      System.out.println("Executing node " + id + " of kind " + kind );
      ...
      else if( kind.equals("arraySet") ) {
         int arrayListIndex = (int)table.retrieve(info);
         int arrayIndex = (int)first.evaluate();
         Double[] array = arrayList.get(arrayListIndex);
         array[arrayIndex] = second.evaluate();
         arrayList.set(arrayListIndex, array);
      }
...
//      System.out.println("Evaluating node " + id + " of kind " + kind );
      ...
      if ( kind.equals("var") ) {
      ...
      else if ( kind.equals("arrayCall") ) {
         int arrayListIndex = (int)table.retrieve(info);
         //System.out.println(arrayListIndex);
         int arrayIndex = (int)first.evaluate();
         //System.out.println(arrayIndex);
         //return arrayList.get(arrayListIndex)[arrayIndex];
         if(arrayList.get(arrayListIndex)[arrayIndex] == null){
            return 0;
         }
         return arrayList.get(arrayListIndex)[arrayIndex];
      }
         ...
         if ( member( funcName, bif0 ) ) {
            ...
            if ( funcName.equals("sqrt") )
               ...
               value = arg1 == 0 ? 1 : 0;
            else if ( funcName.equals("array") ){
               if( arg1 > 0 && (int)arg1 == arg1){
                  arrayList.add(new Double[(int)arg1]);
                  value = (double)arrayList.size() - 1;
               } else {
                  System.out.println("Invalid array declaration");
                  value = -1;
               }
            }
            else {
