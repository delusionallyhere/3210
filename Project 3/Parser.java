...
public class Parser {
   ...
   private Node parseStatement() {
      ...
      if ( token.isKind("string") ) {
        ...
        token = lex.getNextToken();
        errorCheck(token, "end");
        return new Node("for", varName, first, second, third);
        ...
        // String varName = token.getDetails();
        // token = lex.getNextToken();
        // errorCheck(token, "single", "=");// assignment
        // Node first = parseExpr();
        // token = lex.getNextToken();
        // errorCheck(token, "to");
        // second = parseExpr();
        ...
        // token = lex.getNextToken();
        // if(token.isKind("end")){
        //         return new Node("for", varName, first, second, null);
        // }
        // else{
        //       lex.putBackToken(token);
        //       Node third = parseStatements();
      ...
      // --------------->>>   <var> = <expr> or funcCall
         ...
         if ( token.matches("single","=") ) {// assignment
            ...
            errorCheck(token, "single", "=");
            Node second = parseExpr();
            return new Node("arraySet", varName, first, second, null);
         }
         else {
   ...
   private Node parseFactor() {
      ...
      if ( token.isKind("num") ) {
         ...
         if ( token.matches( "single", "(" ) ) {// is a funcCall
            ...
            Node first = parseExpr();
            lex.getNextToken();
            return new Node("arrayCall", name, first, null, null);
         }
         else {// is just a <var>
